<template>
  ...
  <AudioSettings />
  <PlayerSelectPopup v-if="psShow" :show="psShow" @clicked="psClicked" :players="names" @cancelled="psCancelled"
    errorNoPlayer="" :headline="lang.gamePage.selectBlockReveal" />
  ...
  <div class="full-size game-mobile-padding">
    <div class="center-horizontal progress-display" v-if="isPowerupsAllowed === 'true'">
      <div :style="'width: ' + 100 * progressMultiply + 'px; height: 10px'">
        <div :style="'width: ' + 100 * progressMultiply + 'px; height: 10px; background: #eeeeee55'" class="absolute">
        </div>
    ...
    <div class="center-horizontal progress-display-mobile" v-if="isPowerupsAllowed === 'true'">
      <div :style="'width: ' + 100 * (progressMultiply / 2) + 'px; height: 10px'">
        <div :style="'width: ' + 100 * (progressMultiply / 2) + 'px; height: 10px; background: #eeeeee55'"
          class="absolute"></div>
        <div :style="'width: ' + progress * (progressMultiply / 2) + 'px; height: 10px; background: #42b983'"
          class="absolute"></div>
      </div>
      <div>
        <div>{{ progress }}XP</div>
    ...
    <div class="center-horizontal">
      <div v-if="isTutorial && tutPos === 0" class="absolute on-top" style="transform: translate(0px, 170px);">
    ...
    <div class="center-horizontal" v-if="globalDice !== ''">
      <dice :num="globalDice" :mode="globalMode" />
    </div>
      ...
      <div class="absolute center-horizontal" style="width: 100%; margin-top: 50px;">
        <div>
          ...
          <div class="center-horizontal" v-if="loggedDiceNum !== ''">
            <dice :num="loggedDiceNum" :mode="loggedDiceMode" />
          </div>
          <div v-else>
          ...
          <div class="center-horizontal">
            <PowerupView :title="powerup1" :img="pid1" :active="pwActive1" :enabled="pwEnabled1" v-if="powerup1 !== ''"
              @clicked="onPowerupClicked1" @help="helpClicked" />
            <div style="width: 20px" v-if="powerup1 !== '' && powerup2 !== ''"></div>
            <PowerupView :title="powerup2" :img="pid2" :active="pwActive2" :enabled="pwEnabled2" v-if="powerup2 !== ''"
              @clicked="onPowerupClicked2" @help="helpClicked" />
          </div>
          <div class="center-horizontal">
          </div>
          <div class="game-button-layout">
            <div class="center-horizontal">
              ...
              <div v-if="isTutorial && tutPos === 2" class="absolute on-top" style="transform: translate(0px, 50px);">
                <TutorialCard :lang="lang" :text="lang.gamePage.tutorialDrop" :withButton="false" />
              </div>
              <div v-if="isTutorial && tutPos === 3" class="absolute on-top" style="transform: translate(0px, 50px);">
                <TutorialCard :lang="lang" :text="lang.gamePage.tutorialView" :withButton="false" />
              </div>
              <div v-if="isTutorial && tutPos === 5" class="absolute on-top" style="transform: translate(0px, 50px);">
                <TutorialCard :lang="lang" :text="lang.gamePage.tutorialMove" :withButton="false" />
              </div>
              <div v-if="isTutorial && tutPos === 7" class="absolute on-top" style="transform: translate(-170px, 0px);">
                <TutorialCard :lang="lang" :text="lang.gamePage.tutorialLieOrNot" :withButton="true"
                  @clicked="tutNext(8)" />
              </div>
              <div v-if="isTutorial && tutPos === 8" class="absolute on-top" style="transform: translate(-170px, 0px);">
                <TutorialCard :lang="lang" :text="lang.gamePage.tutorialChoose" :withButton="false" />
              </div>



              <div class="center-horizontal" :class="tutPos === 2 || tutPos === 8 ? 'on-top' : ''">
                <GameButton :title="lang.gamePage.dropButton" color="line1" @click="onClickDrop" v-if="isDrop" />
              </div>
              <div class="center-horizontal" :class="tutPos === 5 ? 'on-top' : ''">
                <GameButton :title="lang.gamePage.moveButton" color="line2" @click="onClickMove" v-if="isMove" />
              </div>
            </div>
            <div class="center-horizontal">
              <div class="center-horizontal" :class="tutPos === 3 ? 'on-top' : ''">
                <GameButton :title="lang.gamePage.viewButton" color="line3" @click="onClickView" v-if="isView" />
              </div>
              <div class="center-horizontal" :class="tutPos === 8 ? 'on-top' : ''">
                <GameButton :title="lang.gamePage.revealButton" color="line4" @click="onClickReveal" v-if="isReveal" />
              </div>
            </div>
          </div>
...
export default {
  ...
  components: {
    ...
    HelpPopup,
    PlayerSelectPopup,
    PowerupView, AudioSettings, LangSelection, GameButton, UIButton, Dice, DiceLayout, PlayerView, TutorialCard
  },
  data() {
    return {
      names: [],
  ...
  methods: {
    ...
    updateSettings(message) {
      if (message.player === this.getCookies("username")) {
        if (message.revealButton && this.tutPos === -1) {
          this.tutPos = 6
        }
    ...
    onClickStop() {
      this.setCookies("tutorial", "false")
      let dat = {
        type: "engine",
        func: "stop"
      }
